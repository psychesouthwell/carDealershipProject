...
<form action="DealershipInventory" method="post"></form>
<div>
<c:if test="${cars !=null}" >
  <c:forEach var="finds" items="${finds}">
  	<div class="col-md-4">
      <img src="${finds.pic}" style="width:200px;height:auto;"> <br>
      <h3>
	      <c:out value="${finds.make}"/>
	      <c:out value="${finds.model}"/>
	      <c:out value="${finds.year}"/>
	      <c:out value="${finds.price}"/>
	      <c:out value="${finds.type}"/>
	   </h3>
       <form action="CarDetails">
	   
	   	<input type="hidden" id="vin" name ="vinNumber" value="${finds.vinNumber}">
	    <button type="submit" class="btn btn-info">View Details</button>
	   </form>
   	   <a href="CarDetails?vinNumber=${finds.vinNumber}">
       </a>      
 	</div>
   </c:forEach>
</c:if>
